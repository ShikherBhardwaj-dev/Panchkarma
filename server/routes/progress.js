...
router.put("/patient/:patientId", auth, practitionerAuth, async (req, res) => {
  try {
    ...
    if (wellnessScore !== undefined) {
      const n = Number(wellnessScore);
      if (!Number.isNaN(n)) {
        updateData.wellnessScore = Math.max(0, Math.min(10, n));
      }
    }
...
router.get("/patients", auth, practitionerAuth, async (req, res) => {
  try {
    ...
    const patients = await User.find({
      ...
      .select(
        "name email progress currentStage status progressHistory updatedAt"
      )
      .sort({ updatedAt: -1 });
...
router.get("/my-progress", auth, async (req, res) => {
  try {
    ...
    if (patient.assignedPractitioner) {
      practitioner = await User.findById(patient.assignedPractitioner).select(
        "name email practiceLocation practiceAreas"
      );
    }
